...
public class ASPMain {
	...
	public static void selection(ArrayList<ActivitySelectionProblem> list)
	{  
		System.out.println("Selected Activities:");

        System.out.print(list.get(0).getActivity() + " ");
       
        int lastFinishTime = list.get(0).getFinish();
        
        for (int i = 1; i < list.size(); i++)
        {
            if (list.get(i).getStart() >= lastFinishTime) 
            {
                System.out.print(list.get(i).getActivity() + " ");
                lastFinishTime = list.get(i).getFinish();
            }
        }
	...
	public static void main(String[] args) {
		...
		System.out.println("the sorted activities are:");
		
		for(ActivitySelectionProblem l:list)
			System.out.print(l.getActivity()+" ");
		System.out.println();
